...
SSOT 在 [docs/ssot.md](./docs/ssot.md)。当前实现是 library-first Node.js/TypeScript MVP，默认 memory-only run / goal 调度，可选 durable local replay storage，包含内置 CLI compatibility profiles、强化后的 planner/task-graph validation、真实 stream parser fixtures、fake CLI 集成测试，以及带 redacted diagnostics 和 opt-in real CLI smoke evidence capture 的本地 smoke / query 薄 CLI。
...
- `--mode real` 会执行一次真实的非写入 run，并由 runtime 请求 read-only 行为，但必须同时显式传入 `--allow-real-run` 和 `--agent <id>`。未传 `--cwd` 时使用隔离临时目录，默认 prompt 会要求 agent 不编辑文件。它支持 `--prompt-file`、`--timeout-ms`、`--storage-dir`、`--json`、`--stream jsonl` 和 `--diagnostics`；JSON 输出是 redacted smoke envelope，包含 `classification`、`isolatedCwd`、最终 run record 和 diagnostics。
...
`store-health` 会扫描磁盘 store，不启动 agent。输出包含 run/goal 总数、corrupt manifests、corrupt event logs、partial JSONL tail 及 retained prefix 数、历史 active/interrupted records，以及一致性 warnings。terminal manifest 缺 terminal event、event log 有 terminal event 但 manifest 非 terminal，都会报告 warning；runtime 不会静默替用户改写。

Diagnostics bundle 是单个 run 或 goal 的 redacted JSON 证据包。bundle 包含脱敏 manifest、event summary（不是完整 event payload）、diagnostics、goal task attempt evidence（如存在）以及 environment-safe adapter summary。`--out <file>` 使用 temp file + rename 原子写入。bundle 和 health output 不包含原始损坏 JSONL 行、token、Bearer value、auth-token 环境变量赋值、完整 env dump 或绝对私密路径。
...
| --- | --- | --- | --- | --- |
| Codex CLI | `codex` | stdin | `codex exec --json` | P1-5 本机 real read-only smoke 通过；timeout diagnostics 可分类本地网络/plugin startup 卡顿；transient reconnect events 会解析成 status |
| Claude Code | `claude` | stdin JSONL | `stream-json` | P0-4 已记录 detection baseline；本地 auth 仍 missing |
| OpenCode | `opencode-cli`, `opencode` | stdin | JSON stream | P1-5 本机 `opencode` 1.15.6 非写入 isolated smoke 通过；stdin prompt support 已验证，显式 read-only flags 仍未验证 |
