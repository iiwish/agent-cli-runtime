...
- `--mode real` 会执行一次真实的非写入 read-only run，但必须同时显式传入 `--allow-real-run` 和 `--agent <id>`。未传 `--cwd` 时使用隔离临时目录，默认 prompt 会要求 agent 不编辑文件。它支持 `--prompt-file`、`--timeout-ms`、`--storage-dir`、`--json`、`--stream jsonl` 和 `--diagnostics`；JSON 输出是 redacted smoke envelope，包含 `classification`、`isolatedCwd`、最终 run record 和 diagnostics。
...
| OpenCode | `opencode-cli`, `opencode` | stdin | JSON stream | P1-5 本机 `opencode` 1.15.6 real read-only smoke 通过；stdin prompt support 已在该版本验证 |
