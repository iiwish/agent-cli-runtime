...
export const claudeAdapter: AgentAdapterDef = {
  ...
  compatibility: {
    ...
    knownFlags: [
      { flag: "-p", mapsTo: "printMode" },
      { flag: "--input-format stream-json", mapsTo: "promptTransport" },
      { flag: "--output-format stream-json", mapsTo: "streamFormat" },
      { flag: "--verbose", mapsTo: "requiredForStreamJson" },
      { flag: "--model", mapsTo: "model" },
      { flag: "--add-dir", mapsTo: "extraAllowedDirs" },
      { flag: "--resume", mapsTo: "session.resumeId" },
      { flag: "--session-id", mapsTo: "session.id", needsVerification: true },
