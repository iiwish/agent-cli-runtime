...
状态：0.1.0-alpha.6 corrective alpha target，尚未发布；npm `alpha` 与 `latest` 均指向 alpha.5，alpha.5 GitHub Release tarball parity 已通过但 published verification 因 stale package docs 失败
负责人：local project
最后更新：2026-07-02
...
`0.1.0-alpha.6` 是下一次 corrective alpha target，尚未发布。alpha.6 真实发布、npm dist-tag mutation 和 GitHub Release 创建都需要 fresh release-candidate evidence、`npm run package:docs:check`、`npm publish --dry-run --ignore-scripts --tag alpha`，以及维护者明确授权。后续 alpha.6 发布、beta promotion 或 stable promotion 必须为目标版本重新生成 fresh release evidence，包括 package docs、registry state、GitHub Release parity 和 published verification。

`0.1.0-alpha.5` 已发布到 npm，npm `alpha` 和 `latest` dist-tags 均指向 `0.1.0-alpha.5`。GitHub Release `v0.1.0-alpha.5` 已创建为 prerelease 并上传 npm registry tarball asset，alpha.5 的 `release:post-alpha:verify` tarball parity 已通过。但 alpha.5 的 immutable npm tarball 内含 stale package docs，因此 aggregate published verification（`published:verify` / `published:verify:evidence`）因 `registry_packaged_docs_failed` 失败；alpha.5 不能作为最终 corrective release 验收。

`0.1.0-alpha.4` 已发布到 npm，并曾作为 `alpha` dist-tag 目标。该版本不可变 npm tarball 内含过期的 release-prep package docs，因此 alpha.4 的版本和 dist-tags 以 npm registry metadata 为准。GitHub Release `v0.1.0-alpha.4` 已创建为 prerelease 并上传 npm registry tarball asset，alpha.4 的 `release:post-alpha:verify` tarball parity 已通过。P9-6 fresh main release-candidate evidence、P9-7 publish/post-publish evidence 和 P9-9 alpha.5 post-publish evidence 都作为包外证据记录在 `.release-evidence/`。
