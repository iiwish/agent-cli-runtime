...
状态：P3-4 CI / Release Gate Alignment
负责人：local project
最后更新：2026-06-22
...
- P3-4 当前远端证据状态：`remote_evidence: pending`。需要在当前 commit 上触发新的 `.github/workflows/release-candidate.yml`，下载五个 artifacts，必要时归一化目录，再执行 `npm run release:verify -- --dir <downloaded-artifact-dir>`；不得复用 P2-12 run `27869580048` 作为当前 commit 证据。
