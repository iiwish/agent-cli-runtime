...
状态：0.1.0-alpha.6 已作为 corrective alpha release 发布；npm `alpha` 与 `latest` 均指向 alpha.6，alpha.5 作为 stale-docs incident 保留
负责人：local project
最后更新：2026-07-02
...
`0.1.0-alpha.6` 是已发布的 corrective alpha release。npm `alpha` 与 `latest` dist-tags 均指向 `0.1.0-alpha.6`，GitHub Release `v0.1.0-alpha.6` 已作为 prerelease 存在并带有 npm registry tarball asset，`release:post-alpha:verify` tarball parity 已通过。后续 beta promotion 或 stable promotion 必须为目标版本重新生成 fresh release evidence，包括 package docs、registry state、GitHub Release parity 和 published verification。

`0.1.0-alpha.5` 已发布到 npm，当前 npm `alpha` 和 `latest` dist-tags 均指向 `0.1.0-alpha.6`。GitHub Release `v0.1.0-alpha.5` 已创建为 prerelease 并上传 npm registry tarball asset，alpha.5 的 `release:post-alpha:verify` tarball parity 已通过。但 alpha.5 的 immutable npm tarball 内含 stale package docs，因此 aggregate published verification（`published:verify` / `published:verify:evidence`）因 `registry_packaged_docs_failed` 失败；alpha.5 不能作为最终 corrective release 验收。
...
`0.1.0-alpha.6` 是已发布的 corrective alpha release。
`0.1.0-alpha.5` 已发布到 npm，当前 npm `alpha` 和 `latest` dist-tags 均指向 `0.1.0-alpha.6`，GitHub Release `v0.1.0-alpha.5` 已创建为 prerelease 并上传 npm registry tarball asset，`release:post-alpha:verify` tarball parity 已通过，但 `published:verify:evidence` 因 stale package docs 失败。
