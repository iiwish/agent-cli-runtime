...
状态：0.1.0-alpha.4 release-prep pre-alpha candidate；P9-5 package-visible version/docs drift 需要 P9-6 fresh main evidence
负责人：local project
最后更新：2026-06-30
...
`0.1.0-alpha.4` 是 release-prep package candidate。本分支只准备 package-visible version/docs、contract tests、本地 dry-run 和 branch-local release candidate；不执行真实 `npm publish`、不创建或修改 GitHub Release、不修改 dist-tags、不配置 npm token/trusted publishing、不执行 authenticated real agent run。P9-5 合并后，P9-6 必须对最终 `main` merge commit 重新触发 fresh release-candidate workflow、下载五个 artifacts、运行 `npm run release:verify -- --dir <normalized-downloaded-artifact-dir>`，并把包外证据写入 `.release-evidence/`，之后才进入 human publish decision。
