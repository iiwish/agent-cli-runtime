...
状态：0.1.0-alpha.1 已发布，P6-5 main release-candidate evidence 已证明 P6 offline compatibility gate，P5-4 remote published verification evidence 保持包外记录
负责人：local project
最后更新：2026-06-25
...
当前 `0.1.0-alpha.1` 已发布版本建立在 P3-2/P3-3 daemon-ready installed-package gates、P3-4/P3-5 CI / release-candidate artifact evidence、P3-6 real smoke evidence format、P3-7 API / CLI schema freeze、P3-8 target-SHA remote evidence、P3-9 final alpha dry-run evidence、P3-10 pre-documentation release evidence 之上。

P3-11 current-head evidence boundary 继续约束 post-alpha evidence：易漂移发布证据必须留在包外，并且每个 workflow run 只证明自己的 `headSha`。P5-4 也遵守同一边界：published verification run id、artifact id/digest/size/expiry、target SHA、下载复验命令和 registry 摘要记录在 `.release-evidence/p5-4-published-verification.json`，不写入 npm package。
...
P6-4 针对尚未合入 `origin/main` 的 P6-3 branch target SHA `59b8c00a4ef79356fcba30fb526eab2f158bcdf3` 触发 fresh `.github/workflows/release-candidate.yml`：`gh workflow run release-candidate.yml --ref codex/p6-3-offline-compat-gate`，远端 run `28089574967` 的 `headSha` 精确等于该 target SHA，status/conclusion 为 `completed` / `success`。下载五个 artifacts 后，本地 `npm run release:verify -- --dir <normalized-downloaded-artifact-dir>` 通过，输出 `agent-cli-runtime.releaseVerification.v1`、`ok: true`、diagnostics 为空。下载的 `gate-evidence.json` 包含 `daemon:verify`、`runtime:safety` 和 `compat:real:evidence:verify`；compat gate 输出 schema 为 `agent-cli-runtime.realCompatibilityEvidenceVerification.v1`，被验证 evidence schema 为 `agent-cli-runtime.realCompatibilityEvidence.v1`，diagnostics 只有 `{ count: 0, codes: [] }`。证据摘要记录在 `.release-evidence/p6-4-remote-release-candidate.json`，属于 branch evidence，不是 main evidence。

P6-5 在 P6-1 至 P6-4 通过 PR #4 以 merge commit 合入 `main` 后，针对 `origin/main` merge commit `f2ed4d07103c3a9385d6be66c0befead1784558d` 触发 fresh `.github/workflows/release-candidate.yml`：`gh workflow run release-candidate.yml --ref main`，远端 run `28143647046` 的 `headSha` 精确等于该 main target SHA，status/conclusion 为 `completed` / `success`。下载五个 artifacts 后，本地 `npm run release:verify -- --dir <normalized-downloaded-artifact-dir>` 通过，输出 `agent-cli-runtime.releaseVerification.v1`、`ok: true`、diagnostics 为空、package file count 为 `151`。下载的 `gate-evidence.json` 包含 `daemon:verify`、`runtime:safety` 和 `compat:real:evidence:verify`；compat gate 输出 schema 为 `agent-cli-runtime.realCompatibilityEvidenceVerification.v1`，被验证 evidence schema 为 `agent-cli-runtime.realCompatibilityEvidence.v1`，diagnostics 只有 `{ count: 0, codes: [] }`。证据摘要记录在 `.release-evidence/p6-5-main-release-candidate.json`，属于 main-scoped evidence；它不发布 npm、不创建 npm token、不配置 trusted publishing、不执行 authenticated real agent run。
