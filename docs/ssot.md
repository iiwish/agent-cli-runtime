...
状态：P3-9 Final Alpha Dry-Run & Current-HEAD Release Readiness Lock
负责人：local project
最后更新：2026-06-22
...
当前 P3-9 在 P3-2/P3-3 daemon-ready installed-package gates、P3-4/P3-5 CI / release-candidate artifact evidence、P3-6 real smoke evidence format、P3-7 API / CLI schema freeze、P3-8 target-SHA remote evidence 之上，锁定最终 pre-alpha 当前 HEAD 的 release-candidate 证据和 alpha publish dry-run：上层 daemon 或 product shell 可以嵌入 runtime 来管理 run/goal lifecycle、event replay、cancel/timeout/shutdown、diagnostics、store health/repair、conformance 和 release-artifact verification；仓库用 `npm run daemon:verify` 证明基础嵌入路径可跑，并用 `npm run runtime:safety` 证明单 runtime 长期重复 run/goal、慢事件消费、取消/timeout churn、重复 shutdown、lease close 和 reopen 路径稳定。CI 单 Node release-gates job 执行这两个 gate 和 dogfood；release-candidate artifact 通过 `gate-evidence.json` 和 `release-verification.json` 证明 gate 未漂移；P3-9 远端 run `27942743285` 已在当前证据目标 SHA `a0299a7d81bb614661922bebc8c75496cf0a3d11` 上传五个 artifacts 并通过下载归一化后的本地 `release:verify`；`npm publish --dry-run --ignore-scripts --tag alpha` 本地 dry-run 通过且没有真实发布。P3-7 的 schema inventory、version bump policy、public root boundary 和 failure taxonomy 入口是 [docs/api-schema-contract.md](./api-schema-contract.md)。历史 P3-8 run `27940814340` 只证明目标 SHA `eb8de0f9b1edfa3f94c35a50b31005c5d3c105d4`；历史 P3-5 run `27932628093` 只证明 workflow head SHA `8d7bc2a19c626caa1ad5223acbcd35df34aff18e`。HTTP/API、auth、tenant/team、queue admission、remote worker、UI/artifact、telemetry、database/WAL 仍由上层负责。具体嵌入契约见 [docs/daemon-ready-contract.md](./daemon-ready-contract.md)。
...
- P3-9 不新增 runtime API，不发布 npm，不创建 npm token，不配置 trusted publishing/provenance publish，不执行 authenticated real agent run；目标是把 P3-8 文档证据提交后的当前 HEAD 重新建立最终 pre-alpha release-candidate 证据，并用 alpha dist-tag 完成本地 publish dry-run。
- 证据目标 SHA：`a0299a7d81bb614661922bebc8c75496cf0a3d11` on `main`。触发远端 workflow 前，本地 `main` clean 且领先 `origin/main` 一个文档证据提交；已执行 `git push origin main`，将远端 `main` 从 `eb8de0f9b1edfa3f94c35a50b31005c5d3c105d4` 推进到该证据目标 SHA。推送后本地 `HEAD` 与 `origin/main` 均为 `a0299a7d81bb614661922bebc8c75496cf0a3d11`。
- 本地完整 gate：`npm run typecheck`、`npm run lint`、`npm test`（196 passed、1 skipped）、`npm run build`、`npm run package:check`（151 files checked）、`npm run dogfood`、`npm run daemon:verify`、`npm run runtime:safety`、`npm run release:candidate -- --out-dir /tmp/agent-runtime-p3-9-RGVdwg/release-candidate`、`npm run release:verify -- --dir /tmp/agent-runtime-p3-9-RGVdwg/release-candidate`、`npm audit --omit=dev`（0 vulnerabilities）、`npm pack --dry-run --json --ignore-scripts`、`npm publish --dry-run --ignore-scripts --tag alpha`、`node ./dist/cli/main.js agents --json`、`node ./dist/cli/main.js doctor --json`、`node ./dist/cli/main.js conformance --mode real --agent all --json`、`node ./dist/cli/main.js smoke --mode real --agent codex --json`、`git diff --check` 均通过或按预期安全跳过真实 run；未使用 `--allow-real-run`。
...
- 远端触发：`gh workflow run release-candidate.yml --ref main`，GitHub Actions run `27942743285`，URL `https://github.com/iiwish/agent-cli-runtime/actions/runs/27942743285`，status/conclusion 为 `completed` / `success`，head SHA 为 `a0299a7d81bb614661922bebc8c75496cf0a3d11`。
- Job `Build release candidate artifacts` id `82679850807` 在 `2026-06-22T09:24:09Z` 开始、`2026-06-22T09:25:20Z` 完成；`Run CI gate`、`Run dogfood gate without authenticated real runs`、`Create npm pack artifact and gate evidence without publishing` 及五个 upload artifact steps 均为 `success`。
- GitHub API 返回五个 artifacts：`agent-cli-runtime-tarball` id `7788215886` size `223086` digest `sha256:dc094917ddd33d723eb57f2df1fc17b7811fccec7d39a5cdfccacf07bc460f49` expires `2026-07-06T09:25:10Z`；`agent-cli-runtime-pack-metadata` id `7788216378` size `2006` digest `sha256:d03f369aa26783b8924674b12fba33f6df8a260643c390382523de5d95993701` expires `2026-07-06T09:25:11Z`；`agent-cli-runtime-package-files` id `7788216864` size `963` digest `sha256:5e4769785d0f003a1271c4ac892a8ddbc48ad163b8023cf4082306724c3c6c65` expires `2026-07-06T09:25:12Z`；`agent-cli-runtime-gate-evidence` id `7788217355` size `444` digest `sha256:8bd5251098c5fd6f6057f23ea7f527da63f01765ebf538cf3026a331b30df54f` expires `2026-07-06T09:25:14Z`；`agent-cli-runtime-release-verification` id `7788217990` size `649` digest `sha256:fccc2374836f2e922fecb8b6a573f864314d90153ad52fd88594804faecf76ae` expires `2026-07-06T09:25:15Z`。
- 下载路径 `/tmp/agent-runtime-p3-9-RGVdwg/downloaded` 按 artifact name 分目录；归一化到 `/tmp/agent-runtime-p3-9-RGVdwg/normalized` 后执行 `npm run release:verify -- --dir /tmp/agent-runtime-p3-9-RGVdwg/normalized`。
- 下载 artifact 复验结果为 `schemaVersion: "agent-cli-runtime.releaseVerification.v1"`、`ok: true`、package file count `151`、diagnostics empty、tarball `agent-cli-runtime-0.1.0-alpha.0.tgz`、tarball size `222832` bytes、tarball sha256 `22a588bd3562c1a52315fe5775b381410f57f6e0dfe078d86de235f2178be27c`、npm pack shasum `1a9686f65fb01e0a173cf4c576c130fbb36092e4`。
- `gate-evidence.json` 包含 `daemon:verify` 和 `runtime:safety`，两个 gate 的 `packageSource` 均为 `installed-tarball`，并记录 `noAuthenticatedRealRun: true`、`noNpmPublish: true`、`noNpmToken: true`。
- Package file list 151 项，无 `.reference/`、`tests/`、fixture paths、raw real CLI output、private paths、token-looking values、Bearer values 或 auth env assignments。下载 artifacts 和 tarball 保留在 `/tmp/agent-runtime-p3-9-RGVdwg`，不进入仓库。
