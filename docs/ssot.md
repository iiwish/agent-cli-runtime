...
状态：0.1.0-alpha.1 已发布；0.1.0-alpha.2 为 candidate/prep；P6 offline compatibility gate 已进入 release-candidate 证据链，易漂移 evidence 保持包外记录
负责人：local project
最后更新：2026-06-25
...
当前已发布 npm 版本是 `0.1.0-alpha.1`。当前仓库版本 `0.1.0-alpha.2` 是 alpha.2 release-candidate / prep 状态，只有在 fresh release-candidate evidence 通过、`npm publish --dry-run --ignore-scripts --tag alpha` 通过，并且 maintainer 另行明确授权后，才允许执行真实发布。
...
P6-6 记录 P6-5 合并后的 main HEAD release-candidate artifact 可复验证据，摘要保存在 `.release-evidence/p6-6-main-head-release-candidate.json`。P6-6 证据只在包外，不改变 npm package 内容；P7-1 进入 `0.1.0-alpha.2` candidate/prep 时必须继续遵守同一边界。
