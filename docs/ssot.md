...
状态：0.1.0-alpha.2 已发布；P7-4 post-publish evidence 保持包外记录
负责人：local project
最后更新：2026-06-25
...
当前已发布 npm 版本是 `0.1.0-alpha.2`，并创建了 GitHub pre-release `v0.1.0-alpha.2`。当前 npm dist-tags 为 `alpha -> 0.1.0-alpha.2` 和 `latest -> 0.1.0-alpha.1`。P7-4 post-publish evidence、fresh main release-candidate evidence、registry verification、installed-package smoke 和 GitHub Release verification 均作为包外证据记录。
...
P6-6 记录 P6-5 合并后的 main HEAD release-candidate artifact 可复验证据，摘要保存在 `.release-evidence/p6-6-main-head-release-candidate.json`。P6-6 证据只在包外，不改变 npm package 内容；P7-4 记录 `0.1.0-alpha.2` 的 post-publish evidence，摘要保存在 `.release-evidence/p7-4-alpha-2-post-publish.json`，并继续遵守同一边界。
