...
This project is still **pre-alpha / developer preview**. Version `0.1.0-alpha.4` is published on npm with the `alpha` dist-tag, while `latest` remains on `0.1.0-alpha.1`. The immutable alpha.4 npm tarball contains stale release-prep package docs, so npm registry metadata is authoritative for alpha.4 version and dist-tags. GitHub Release `v0.1.0-alpha.4` is not created yet, so GitHub Release tarball parity evidence for alpha.4 remains open.
