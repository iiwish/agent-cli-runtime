...
- `.release-evidence/p8-7-main-release-candidate.json` is the current repo-only main-scoped remote release-candidate summary; it is generated with `npm run release:main-candidate:evidence -- --stage P8-7 --out .release-evidence/p8-7-main-release-candidate.json`, records `releaseTargetSha`, local strict matrix verification, local strict release artifact verification, fresh `release-candidate.yml --ref main` metadata, exact five-artifact metadata with `expired: false`, P8-6 normalized five-artifact download verification, target-SHA-only evidence scope, `mainEvidence: true`, `branchEvidence: false`, and the same no-authenticated-run/no-publish/no-token boundary without raw logs, raw stdout/stderr, prompts, private paths, local temp paths, resolved executable paths, tokens, or auth environment values;
...
- `npm run release:main-candidate:evidence` writes reusable P8 main-scoped remote release-candidate summaries after `releaseTargetSha`, `origin/main`, remote workflow `headSha`, exact artifact metadata, and downloaded `release:verify` all match; `--stage <stage>` selects the evidence stage and output identity;
