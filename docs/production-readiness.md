...
This project is still **pre-alpha / developer preview**. Version `0.1.0-alpha.6` is the published corrective alpha release. npm `alpha` and `latest` dist-tags both point at `0.1.0-alpha.6`; GitHub Release `v0.1.0-alpha.6` exists as a prerelease with the npm registry tarball asset, and GitHub Release tarball parity verification passes for alpha.6. Future beta or stable promotion requires fresh release-candidate evidence, package-docs verification, dry-run, and explicit maintainer authorization. Version `0.1.0-alpha.5` is published on npm as a stale-docs incident. GitHub Release `v0.1.0-alpha.5` exists as a prerelease with the npm registry tarball asset, and GitHub Release tarball parity verification passes for alpha.5. The immutable alpha.5 npm tarball contains stale package docs, so aggregate `published:verify` / `published:verify:evidence` fails with `registry_packaged_docs_failed`. Version `0.1.0-alpha.4` is published on npm and was previously on the `alpha` dist-tag. The immutable alpha.4 npm tarball contains stale release-prep package docs, so npm registry metadata is authoritative for alpha.4 version and dist-tags. GitHub Release `v0.1.0-alpha.4` exists as a prerelease with the npm registry tarball asset, and GitHub Release tarball parity verification passes for alpha.4. Future beta promotion or stable promotion requires fresh release evidence for the target version.

Version `0.1.0-alpha.3` is the previous corrective pre-alpha release for package consumers. Version `0.1.0-alpha.2` was published with fresh main release-candidate evidence, real publish evidence, registry verification, installed-package CLI smoke, and GitHub Release verification, but its immutable npm tarball contains stale pre-publish package docs. Version `0.1.0-alpha.1` remains published as an earlier alpha and has GitHub pre-release `v0.1.0-alpha.1`. Version `0.1.0-alpha.0` is deprecated because that immutable tarball contains stale pre-publish status text. npm registry metadata and GitHub Releases are the source of truth for available versions and dist-tags. P3-1 froze daemon-ready execution-kernel contracts for embedders in [docs/daemon-ready-contract.md](./daemon-ready-contract.md); P3-7 freezes the API / CLI schema inventory and versioning policy in [docs/api-schema-contract.md](./api-schema-contract.md); P8-2 records real Codex / Claude Code / OpenCode compatibility as a repo-only matrix, and release review is bound through matrix target SHA, freshness, dirty-policy verification, local strict release artifacts, branch/main evidence scope, artifact normalization, fresh main remote artifact verification, package-content equivalence decisions, and stage-neutral main release-candidate evidence without launching authenticated real agent runs.

Detailed run, artifact, and package-content equivalence evidence for P6-4 through P8-9 is recorded outside the npm package under `.release-evidence/`.

Volatile target-SHA evidence stays out of the npm package. Fresh run ids, artifact ids, artifact digests, tarball hashes, pack hashes, downloaded verification paths, raw logs, raw CLI output, full prompts, and token-looking values belong under `.release-evidence/` or durable GitHub Release assets. Packaged docs keep stable release rules, the alpha.2 stale package-docs incident, the alpha.3 corrective history, the alpha.4 npm publish state, the stale alpha.4 release-prep package docs incident, the alpha.4 GitHub Release asset/parity state, the alpha.5 published state and stale-docs incident, the alpha.6 published corrective release state, package-docs verification, and the human-gated boundary for any future registry or release mutation. P5-1 adds a published-package daemon consumer harness for the already published registry package: it installs from npm, uses fake CLIs only, and verifies daemon-style lifecycle coverage without touching local `dist/` or publishing a new version. The post-alpha path does not configure trusted publishing, claim provenance, or add daemon/API server/database/WAL/remote-worker/UI/telemetry/artifact layers.
...
- `npm run release:main-candidate:evidence -- --stage <stage>` writes stage-neutral repo-only main-scoped release-candidate summaries with `schemaVersion: "agent-cli-runtime.mainReleaseCandidateEvidence.v1"`; current phase labels such as `P9-2` are valid, P8 summaries are retained only as historical evidence, and current fresh evidence requires `releaseTargetSha == origin/main == workflow headSha`, local strict matrix verification, local strict release artifact verification, exact five non-expired `sha256:` artifacts, normalized downloaded artifact verification, `mainEvidence: true`, `branchEvidence: false`, and the same no-authenticated-run/no-publish/no-token boundary without raw logs, raw stdout/stderr, prompts, private paths, local temp paths, resolved executable paths, tokens, or auth environment values;
- `npm run release:package-content:verify -- --base-ref <release-target-sha> --head-ref <sha-or-ref>` emits `agent-cli-runtime.packageContentEquivalence.v1` and compares npm package-visible file lists plus per-file content hashes from temporary git worktrees; package-external `.release-evidence/`, tests, and repo-only script changes can record `evidenceOnlyDrift: true` with `freshReleaseCandidateRequired: false`, while README, packaged docs, package.json, dist, types, bin, examples, and other package-visible content changes require fresh release-candidate evidence before the head ref is treated as a release target;
...
- `npm run release:artifacts:normalize -- --download-dir <gh-download-dir> --out-dir <normalized-artifact-dir>` normalizes the five downloaded release-candidate artifact directories into one verifier-ready directory, emits `agent-cli-runtime.releaseArtifactNormalization.v1`, and rejects missing, duplicate, unknown, or misplaced files without exposing local absolute paths;
- `npm run release:verify` validates local or downloaded release artifacts and emits stable redacted JSON;
- `npm run release:main-candidate:evidence` writes reusable main-scoped remote release-candidate summaries after `releaseTargetSha`, `origin/main`, remote workflow `headSha`, exact artifact metadata, and downloaded `release:verify` all match; `--stage <stage>` selects the evidence stage and output identity, and P8 files are summarized as historical non-current evidence;
- `npm run release:post-alpha:verify` compares npm registry and GitHub Release tarballs, allowing raw gzip hash differences only when unpacked package content is identical; for alpha.5, GitHub Release `v0.1.0-alpha.5` exists with the npm registry tarball asset and tarball parity passes, while aggregate `published:verify` / `published:verify:evidence` fails for the alpha.5 registry package because packaged docs are stale;
...
- `docs/release-publish-runbook.md` records current post-alpha registry state, the future publish command path, 2FA/trusted publishing/provenance decisions, dist-tag checks, and rollback boundaries without configuring trusted publishing or storing credentials;
