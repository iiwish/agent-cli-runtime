...
This project is still **pre-alpha / developer preview**. Version `0.1.0-alpha.5` is the corrective alpha target to replace stale alpha.4 package docs for consumers; it requires fresh release-candidate evidence before any explicit maintainer authorization for real publish. Version `0.1.0-alpha.4` is published on npm with the `alpha` dist-tag, while `latest` remains on `0.1.0-alpha.1`. The immutable alpha.4 npm tarball contains stale release-prep package docs, so npm registry metadata is authoritative for alpha.4 version and dist-tags. GitHub Release `v0.1.0-alpha.4` exists as a prerelease with the npm registry tarball asset, and GitHub Release tarball parity verification passes for alpha.4. After any authorized publish of alpha.5, rerun `published:verify` and `published:verify:evidence` against the npm registry package before accepting the corrective release.
...
- `npm run release:post-alpha:verify` compares npm registry and GitHub Release tarballs, allowing raw gzip hash differences only when unpacked package content is identical; for alpha.4, GitHub Release `v0.1.0-alpha.4` exists with the npm registry tarball asset and tarball parity passes, while aggregate `published:verify` / `published:verify:evidence` remains blocked by stale release-prep package docs inside the immutable alpha.4 npm tarball;
