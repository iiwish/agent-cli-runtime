...
This project is still **pre-alpha / developer preview**. Version `0.1.0-alpha.4` is the release-prep package candidate and is not published by this branch. After merge, P9-6 must generate fresh main release-candidate evidence for the final `main` merge commit before any human publish decision.
...
Volatile target-SHA evidence stays out of the npm package. Fresh run ids, artifact ids, artifact digests, tarball hashes, pack hashes, downloaded verification paths, raw logs, raw CLI output, full prompts, and token-looking values belong under `.release-evidence/` or durable GitHub Release assets. Packaged docs keep stable release rules, the alpha.2 stale package-docs incident, the alpha.3 corrective history, the alpha.4 release-prep boundary, package-docs verification, and the human-gated boundary for any registry mutation. P5-1 adds a published-package daemon consumer harness for the already published `agent-cli-runtime@0.1.0-alpha.1`: it installs from the npm registry, uses fake CLIs only, and verifies daemon-style lifecycle coverage without touching local `dist/` or publishing a new version. The post-alpha path does not configure trusted publishing, claim provenance, or add daemon/API server/database/WAL/remote-worker/UI/telemetry/artifact layers.
